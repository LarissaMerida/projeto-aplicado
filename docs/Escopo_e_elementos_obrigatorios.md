...
* [X] O acesso à nuvem utiliza boas práticas (uso de chave SSH e Fail2Ban configurado para a porta 22) (Eixo 1).
* [X] O código está versionado em um repositório público no GitHub e a conta está devidamente configurada (Eixo 2).
* [X] O `.gitignore` está configurado e não há chaves/senhas expostas no código (Eixo 2).
...
* [X] O fluxo de implantação está devidamente automatizado com CI/CD utilizando o GitHub Actions (Integração e Entrega Contínuas).
